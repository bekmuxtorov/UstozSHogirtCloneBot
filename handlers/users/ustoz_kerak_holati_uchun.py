...
async def bot_echo(message: types.Message):
    text = """<b>Ustoz topish uchun ariza berish</b> \n\n Hozir sizga birnecha savollar beriladi. 
Har biriga javob bering.
Oxirida agar hammasi to`g`ri bo`lsa, HA tugmasini bosing va arizangiz Adminga yuboriladi."""
    await message.answer(text=text)
    await message.answer(text='<b>Ism familiyangizni kiriting?</b>')
    await Holatlar.ustoz_ism_holati.set()


@dp.message_handler(state=Holatlar.ustoz_ism_holati)
async def bot_echo(message: types.Message, state: FSMContext):
    ism = message.text
    await state.update_data({"ism": ism})
    text = "🕑 <b>Yosh:</b>\n\nYoshingizni kiriting?\nMasalan, 19"
    await message.answer(text=text)
    await Holatlar.ustoz_yosh_holati.set()

@dp.message_handler(state=Holatlar.ustoz_yosh_holati)
async def bot_echo(message: types.Message, state: FSMContext):
    yosh = message.text
    await state.update_data({"yosh": yosh})
    text = "📚<b>Texnalogiya:</b>\n\nTalab qilinadigan texnologiyalarni kiriting?\nTexnologiya nomlarini vergul bilan ajrating. Masalan,\n\n <i>Java, C++, C#</i>"
    await message.answer(text=text)
    await Holatlar.ustoz_texnalogiya_holati.set()

@dp.message_handler(state=Holatlar.ustoz_texnalogiya_holati)
async def bot_echo(message: types.Message, state: FSMContext):
    texnalogiya = message.text
    await state.update_data({"texnalogiya": texnalogiya})
    text = "📞<b>Aloqa:</b>\n\nBog`lanish uchun raqamingizni kiriting?\nMasalan, +998 90 123 45 67"
    await message.answer(text=text)
    await Holatlar.ustoz_aloqa_holati.set()


@dp.message_handler(state=Holatlar.ustoz_aloqa_holati)
async def bot_echo(message: types.Message, state: FSMContext):
    aloqa = message.text
    await state.update_data({"aloqa": aloqa})
    text = "🌐<b>Hudud:</b>\n\nQaysi hududdansiz? \nViloyat nomi, Toshkent shahar yoki Respublikani kiriting."
    await message.answer(text=text)
    await Holatlar.ustoz_hudud_holati.set()


@dp.message_handler(state=Holatlar.ustoz_hudud_holati)
async def bot_echo(message: types.Message, state: FSMContext):
    hudud = message.text
    await state.update_data({"hudud": hudud})
    text = "💰<b>Narxi:</b>\n\nTo'lov qilasizmi yoki Tekinmi? \nKerak bo`lsa, Summani kiriting?"
    await message.answer(text=text)
    await Holatlar.ustoz_narx_holati.set()

@dp.message_handler(state=Holatlar.ustoz_narx_holati)
async def bot_echo(message: types.Message, state: FSMContext):
    narx = message.text
    ...
    await Holatlar.ustoz_kasb_holati.set()

@dp.message_handler(state=Holatlar.ustoz_kasb_holati)
async def bot_echo(message: types.Message, state: FSMContext):
    kasb = message.text
    ...
    await Holatlar.ustoz_vaqt_holati.set()


@dp.message_handler(state=Holatlar.ustoz_vaqt_holati)
async def bot_echo(message: types.Message, state: FSMContext):
    vaqt = message.text
    await state.update_data({"vaqt": vaqt})
    text = "🔎<b>Maqsad:</b>\n\nMaqsadingizni qisqacha yozib bering."
    await message.answer(text=text)
    await Holatlar.ustoz_maqsad_holati.set()

@dp.message_handler(state=Holatlar.ustoz_maqsad_holati)
async def bot_echo(message: types.Message, state: FSMContext):
    maqsad = message.text
...
@dp.message_handler(state=Holatlar.ustoz_tasdiqlash_holati, text='Ha')
